...
Попутно он снимает метрики системы и сети, держит канал команд с панелью
(терминал, скрипты, списки блокировок) и ловит торренты — тегом роутинга
Xray, а по желанию ещё и разбором трафика через nDPI, см. «Детект
торрентов».
...
По умолчанию торренты ловит сам Xray: правило роутинга с
`"protocol": ["bittorrent"]` вешает на соединение тег `TORRENT`, агент
видит его в логе и шлёт событие в панель.

Ловится так, увы, немногое. Xray опознаёт BitTorrent по открытому
рукопожатию `BitTorrent protocol` в начале соединения, а современные
клиенты по умолчанию шифруют поток (MSE/PE) и работают через DHT и uTP
поверх UDP — до рукопожатия дело просто не доходит. На тег попадаются в
основном старые клиенты с выключенным шифрованием.

Второй источник — [nDPI](https://github.com/ntop/nDPI): у него есть
эвристики на зашифрованный BitTorrent, на DHT-пакеты и на uTP.

### Как включить

Тумблер **«Детект торрентов через nDPI»** в панели: Настройки →
Нарушения. Больше ничего делать не нужно — ни ставить пакеты, ни лезть в
`.env` на нодах.

Работает это так: демон `nDPId` уже лежит внутри образа агента, поэтому
включение сводится к запуску пары процессов. Агент живёт в сети хоста с
`privileged` (это нужно и для сетевых метрик), так что интерфейс он
слушает напрямую. Интерфейс выбирается по маршруту по умолчанию.

Настройка отправляется агенту заново при каждом подключении, так что
переживает перезапуск ноды. Агент отвечает не «принято», а состоянием:
поднялся ли демон, подключён ли сокет, сколько вердиктов пришло — по
нему видно разницу между «работает» и «включено, но что-то не так».

### Тонкая настройка (обычно не нужна)

```bash
AGENT_NDPI_ENABLED=true          # включить, не дожидаясь команды из панели
AGENT_NDPI_MANAGE_DAEMON=true    # false — демон уже поднят на ноде снаружи
...
Если сокет по указанному пути уже существует, агент считает, что демон
поднят снаружи, и своих процессов не плодит.

### Как вердикт попадает к пользователю

nDPI видит трафик уже после NAT — от имени ноды, и о том, чей это клиент,
не знает ничего. Зато адрес назначения у него и у Xray один и тот же: по
нему агент и связывает вердикт со строкой лога, где есть пользователь.

В событии, которое уходит в панель, есть поле `detected_by`:
`xray_routing` — сработал тег роутинга, `ndpi` — вердикт nDPI. Признаки
разной твёрдости, и по нему видно, на чём именно попался человек.

⚠️ nDPI разбирает трафик на интерфейсе ноды — это шаг дальше, чем чтение
метаданных из лога Xray. Включайте осознанно.
...
- NOTIFICATIONS_TOPIC_BACKUPS=${NOTIFICATIONS_TOPIC_BACKUPS:-}
```
