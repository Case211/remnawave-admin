...
## Детект торрентов: nDPI (необязательно)
...
Проблема в том, что Xray опознаёт BitTorrent по открытому рукопожатию
`BitTorrent protocol` в начале соединения. Современные клиенты по
умолчанию шифруют поток (MSE/PE), работают через DHT и uTP поверх UDP —
до рукопожатия дело не доходит, и такой трафик проходит мимо. На тег
попадаются в основном старые клиенты с выключенным шифрованием.

Второй источник — [nDPI](https://github.com/ntop/nDPI): у него есть
эвристики на зашифрованный BitTorrent, на DHT-пакеты и на uTP. Демон
`nDPId` гоняет библиотеку по трафику интерфейса и раздаёт вердикты
JSON-строками через UNIX-сокет; агент их читает и сопоставляет с логом
Xray по адресу назначения — сам nDPI видит трафик уже после NAT и о том,
чей это клиент, не знает.

### Установка nDPId

```bash
git clone https://github.com/utoni/nDPId.git
cd nDPId && mkdir build && cd build
cmake .. -DBUILD_NDPI=ON && make -j"$(nproc)"

# демон инспекции + раздатчик вердиктов
./nDPId -i eth0 -c /tmp/ndpid-collector.sock
./nDPIsrvd -c /tmp/ndpid-collector.sock -s /tmp/ndpid-distributor.sock
```

Агент подключается к **distributor**-сокету (`nDPIsrvd`), а не к
collector-сокету самого `nDPId`.

### Включение из панели (проще)

Тумблер **«Детект торрентов через nDPI»** в настройках панели
(Настройки → Нарушения) рассылает состояние агентам по каналу команд —
править `.env` на каждой ноде не нужно. Настройка отправляется заново при
каждом подключении агента, так что переживает и перезапуск ноды.

Агент отвечает состоянием, а не просто «принято»: чтение сокета
включается всегда, а демона на ноде может не быть. По ответу видно
разницу между «включено и работает» и «включено, но сокета нет».
...
Агент запущен в контейнере, поэтому сокет нужно пробросить внутрь:

```yaml
    volumes:
      - /var/log/remnanode:/var/log/remnanode:ro
      - ./logs:/app/logs
      - /tmp/ndpid-distributor.sock:/tmp/ndpid-distributor.sock
```

Если сокета нет или демон не поднят, агент работает как раньше — на теге
роутинга: подключение к сокету повторяется в фоне и основному циклу не
мешает.

В событии, которое уходит в панель, есть поле `detected_by`:
`xray_routing` — поймал тег роутинга, `ndpi` — вердикт nDPI. Признаки
разной твёрдости, и по нему видно, на чём именно попался пользователь.

⚠️ nDPI смотрит трафик на интерфейсе ноды — это шаг дальше, чем чтение
метаданных из лога Xray. Решение осознанное: включайте, если понимаете,
что и зачем инспектируете.
