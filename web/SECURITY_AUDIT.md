...
|-----------|--------|-----------|
| P0 | Rate limiting на auth эндпоинты | Низкая |
| P0 | Убрать Telegram ID из ответов ошибок | Низкая |
| P1 | Token blacklist при логауте | Средняя |
| P1 | HttpOnly cookies для JWT | Высокая |
| P1 | Whitelist JWT алгоритмов | Низкая |
| P2 | CSP и security headers | Низкая |
| P2 | Ограничить CORS методы/заголовки | Низкая |
| P2 | WebSocket token через handshake | Средняя |
| P3 | SRI для Telegram widget | Низкая |
| P3 | Кастомные модалки для удаления | Средняя |
