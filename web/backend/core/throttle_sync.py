...
Режет на ноде шейпер агента (1.9.0+): персональный лимит — по адресу, в обе
стороны и на любом порту; общий потолок ноды при этом тоже действует, и
клиенту достаётся меньший из двух.
"""
import asyncio
import json
import logging
...
# Что каждой ноде уже отправлено: {node_uuid: отпечаток списка}. Шейпер
# меняет адреса в карте без перезагрузки, но старые агенты переставляли корень
# интерфейса целиком — дёргать ноду незачем, пока список не изменился.
