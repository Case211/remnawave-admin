...
| 12.1.2 | Извлечение строк | ✅ | Все hardcoded строки → `t('key')`. 14 страниц + 10 компонентов. Namespace-разделение: common, nav, pages/*, components/* |
...
| 12.2.3 | Backend: i18n ошибок | ⏳ | Коды ошибок вместо текстовых сообщений, перевод на фронте (требует изменений бэкенда) |
| 12.2.4 | Автоматизации: шаблоны уведомлений | ⏳ | Шаблоны Telegram/webhook с поддержкой языка (требует изменений бэкенда) |
