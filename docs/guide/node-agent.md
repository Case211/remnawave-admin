...
1. **Сервера → Ноды панели** → меню нужной ноды → **Токен агента** → **Установить агент**
...
Токен генерируется в панели (**Сервера → Ноды панели → меню ноды → Токен агента**) или в боте. Показывается один раз.
...
## Свои серверы без Xray

Агент умеет жить и на сервере, которого нет в панели Remnawave: бот, сама панель, база. Он собирает
CPU, память, диск и сеть, держит канал команд (терминал, скрипты), а лог Xray просто не находит и
работает в режиме «только метрики».

1. **Сервера → Fleet → Добавить сервер**, укажите название и адрес.
2. Скопируйте команду установки из ответа и выполните на сервере: она та же, что для нод, только UUID
   выдан админкой, а не панелью Remnawave.
3. Сервер появится во Fleet с пометкой «Сервер» после первого отчёта агента. Правила алертов по CPU,
   RAM, диску и «не отвечает» распространяются и на него.

Такие серверы не попадают во вкладку «Ноды панели» и в счётчики нод на дашборде, синк с панелью их не трогает.
Связь считается по метрикам: агент молчит дольше пяти минут — сервер офлайн. Удаляется он кнопкой
в карточке Fleet; контейнер агента на сервере после этого нужно остановить самому.
