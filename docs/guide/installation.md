...
Нужен для анти-абуза, метрик хоста, терминала и детекта торрентов. Ставится одной командой, которую панель собирает сама: **Сервера → Ноды панели → меню ноды → Токен агента → Установить агент**.
