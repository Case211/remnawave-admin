...
### 🌐 Веб-панель
- 📊 Дашборд с трендами онлайна, нарушений и очередью коллектора
- 👥 Управление пользователями, нодами, хостами + bulk-операции
...
- 📈 Расширенная аналитика — топ юзеров, тренды (минутная грануляция 24ч), трафик по нодам, P2P/Torrent
- 🤖 Хаб автоматизаций — конструктор правил, расписания, шаблоны, логи выполнения
- 🔌 **Plugin Manager** — установка и обновление платных плагинов прямо из UI (`/admin/plugins`)
- ⚙️ Настройки в реальном времени без рестарта (приоритет: БД > .env > defaults)
- 🔐 Авторизация: Telegram Login Widget, пароль, 2FA (TOTP), биометрия + RBAC (роли и права)
...
- 🔔 Уведомления с маршрутизацией по топикам Telegram + push, email, webhook

### 📈 Мониторинг и наблюдаемость (с 2.14)
- 📡 `/metrics` endpoint в Prometheus text-формате — RPS, latency-перцентили, пул PostgreSQL
- 🔢 30+ кастомных gauges: users by status, traffic buckets, expiring buckets, HWID by platform, per-node CPU/RAM/disk/IO/uptime, sync lag, violations by action
- 🐳 Prometheus встроен опциональным compose-профилем: `docker compose --profile monitoring up -d`
- 📊 5 готовых Grafana-дашбордов в `docs/grafana/` — Overview, HTTP & Performance, Users & Subscriptions, Nodes, Anti-abuse & Sync — кросс-линкованы
- 🔐 Защита `/metrics` Bearer-токеном (template подставляется из `.env` автоматически)

### 📱 Мобильное приложение (Android)
- Тёмный HUD-стиль, виджет 4×2 на главный экран (Glance)
- FCM push с per-event подписками, deeplink-навигация
- Поддержка нескольких профилей панели, биометрический вход
- Аналитика и тренды в карманном формате

### 🛡 Anti-Abuse система
- 🔍 7 анализаторов: временной, географический, ASN, профиль поведения, устройства, HWID, User-Agent
- 🌍 Детекция «невозможных путешествий», распознавание 60+ российских агломераций
- ⚡ Автоматические действия по порогам скоринга (warn / soft-block / temp-block / hard-block)
- 🔥 Мониторинг скорости потребления трафика с настраиваемыми порогами + автоблокировка
- 🧲 P2P/Torrent детекция и аналитика (интеграция с torrent-blocker)
- 📋 Аудит-репорты нарушений в Telegram с inline-действиями (block, drop connections, whitelist)
...
- 📥 Приём входящих писем (встроенный SMTP-сервер), STARTTLS на обоих портах
- 🛡 Проверка подлинности входящих: SPF, DKIM, выравнивание DMARC, спам-оценка
- 📎 Вложения — хранение и скачивание; ответ из интерфейса с сохранением ветки переписки
- 🚫 Стоп-лист: жёсткие отказы доставки и отписки закрывают адрес автоматически
- 📊 Разбор DMARC-отчётов — видно, кто рассылает письма от имени ваших доменов
...
- 📊 Дашборд с метриками CRM, клиентами, подписками, реферальной системой
- 🎫 Промокоды, маркетинговая аналитика
- 🚪 Личный кабинет клиента (`web/cabinet`) с авторизацией через JWT
...
- 🧪 1200+ backend тестов + 260+ vitest, Playwright E2E, CI/CD workflows
...
Нажмите **"DNS Records"** у домена — система покажет записи для добавления у DNS-провайдера:
...
> ⚠️ Проверка MX сверяет, **куда** ведёт запись, а не только её наличие. Если домен переехал от прежнего хостера, его MX часто остаются в зоне и молча забирают всю входящую почту — отправители получают отказ вроде `550 Disabled`, а панель об этом не знает. Удалите чужие MX перед добавлением своей.
...
HTTP API (`/api/v2/mailserver/*`) идёт через nginx/Caddy как обычно. **SMTP проксировать через HTTP-прокси нельзя** — это L4 TCP. Самое простое: прокинуть порт 25 → 2525 в `docker-compose.yml` мимо прокси. Если нужен общий entrypoint — настроить nginx `stream {}` или caddy-l4.

Требования:
- MX, SPF, PTR записи указывают на публичный IP сервера
- PTR-запись настраивается у хостера (улучшает доставляемость)
- Многие облака (AWS, GCP, Azure) блокируют :25 — используйте Hetzner / OVH / DigitalOcean

#### Шифрование (TLS)

На обоих почтовых портах поднимается STARTTLS. Сертификат берётся из настроек:
...
Если путь не задан — выписывается самоподписанный сертификат в том `mail_certs`. Для приёма почты этого достаточно: почтовые серверы шифруют соединение, не проверяя сертификат. А вот на порту 587 (SMTP Submission) клиенты будут ругаться, поэтому туда лучше положить настоящий — например, смонтировать сертификат reverse-proxy.

> ⚠️ Пока сертификата нет, настройка **«Требовать TLS на порту 587»** не действует, и логин с паролем уходят по незашифрованному соединению (AUTH PLAIN — это base64, а не шифрование). В логах при старте об этом есть предупреждение.

#### Проверка входящих писем

Каждое полученное письмо проходит SPF, DKIM и DMARC; результат виден в списке и в карточке письма. Набравшие порог помечаются подозрительными и в общий список не попадают — для них отдельный фильтр. Настройка **«Отклонять подозрительные письма»** заставляет отказывать прямо на приёме: тогда настоящий отправитель хотя бы получит уведомление о недоставке вместо тишины.
...
- письмо на `unsubscribe@<домен>` — адрес из заголовка `List-Unsubscribe`, который панель ставит в каждое письмо.
...
Почтовые системы раз в сутки присылают сводку на адрес из записи `_dmarc` — кто и с каких адресов слал письма от имени домена и прошли ли они проверку. Панель распаковывает вложение (`.gz`/`.zip`), разбирает XML и показывает результат на вкладке **DMARC**. В разделе «Отправители без подписи» виден и чужой спуфинг, и собственный сервис, забывший про DKIM.

#### Хранение

По умолчанию входящие хранятся вечно, история отправки — 90 дней (ожидающие отправки письма не удаляются). Меняется в настройках; `0` — хранить без ограничения.

#### Проверка

1. Активируйте домен → **Compose → Send Test** → проверь `Queue` (статус `sent`)
2. Если активный домен есть — система уведомлений сама использует встроенный сервер (fallback на SMTP relay)
...
### Шаг 9️⃣ — Мониторинг (опционально)

Панель отдаёт Prometheus-метрики на `/metrics` без дополнительной настройки. Если нужен встроенный Prometheus + готовые дашборды Grafana:

```bash
# Защити /metrics токеном (рекомендуется в проде)
echo "METRICS_AUTH_TOKEN=$(openssl rand -hex 32)" >> .env

# Подними Prometheus как часть compose
docker compose --profile monitoring up -d
```

Импорт 5 дашбордов из `docs/grafana/` в свою Grafana (Dashboards → New → Import → Upload JSON file):
`overview.json` → `http-performance.json` → `users-subscriptions.json` → `nodes.json` → `anti-abuse-sync.json`.

Дашборды кросс-линкованы — на каждом сверху строка переходов в соседние.

> 📖 Подробнее, каталог метрик и интеграция с VictoriaMetrics / vmagent: [docs/README.md](docs/README.md)
...
```
remnawave-admin/
├── src/                        # Telegram-бот (aiogram)
│   ├── handlers/               # users, nodes, hosts, billing, violations, blocked_ips, ...
│   ├── keyboards/              # Inline-клавиатуры
│   ├── services/               # Webhook, report scheduler, health check
│   └── utils/                  # i18n, форматирование, push notifications
...
│   ├── violation_detector.py   # 7-факторный анализ нарушений
│   ├── push_service.py         # FCM push для мобильных
│   └── metrics.py              # Prometheus counters
├── web/
│   ├── frontend/               # React + TypeScript + Tailwind + shadcn/ui
│   ├── backend/                # FastAPI бэкенд (RBAC, аналитика, plugin manager)
│   └── cabinet/                # Клиентский кабинет (Bedolaga)
├── node-agent/                 # Агент сбора данных с нод (Command Channel, скрипты)
├── monitoring/                 # Prometheus template + entrypoint
├── docs/grafana/               # 5 готовых дашбордов Grafana
├── alembic/                    # Миграции БД
├── locales/                    # Локализация (ru, en)
└── docker-compose.yml          # Docker Compose + опциональный --profile monitoring
```
