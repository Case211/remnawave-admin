...
class UserAgentClassification(Enum):
    """Классификация User-Agent подписочного запроса."""
    VALID = "valid"              # whitelist — легитимный клиент
    LINK_IN_UA = "link_in_ua"    # vless://, https:// — подписочная ссылка вставлена как UA (двойной туннель)
    BOT_LIBRARY = "bot_library"  # curl, Go-http-client, python-requests — скрипт/бот
    STUB = "stub"                # Mozilla/5.0 голый, без продолжения
    EMPTY = "empty"              # UA не передан
    UNKNOWN = "unknown"          # не whitelist и не blacklist — возможно новый клиент


@dataclass
class SuspiciousAgent:
    """Одна подозрительная запись из Subscription Request History."""
    request_id: Optional[int]
    user_agent: str
    request_ip: Optional[str]
    request_at: Optional[str]  # ISO datetime string
    classification: str  # UserAgentClassification.value


@dataclass
class UserAgentScore:
    """Скор анализа User-Agent подписочных запросов."""
    score: float
    reasons: List[str]
